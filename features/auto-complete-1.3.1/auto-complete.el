...
;; Version: 1.3.1.SMALLWORLD (remove X window specifics and allow multiple sources with prefix)
...
(defcustom ac-quick-help-prefer-tooltip t  ;; AJM remove X window specifics
  "Prefer a tooltip rather than overlay popup for displaying quick help."
  :type 'boolean
  :group 'auto-complete)
...
(defun ac-prefix (requires ignore-list)
  (loop with current = (point)
	...
	;; AJM Allow for multiple sources with "prefix" to provide candidates
        ;;if (null prefix-def)
        do
...
(defun ac-quick-help (&optional force)
  ...
  (when (and (or force (null this-command))
             ...
             (null ac-quick-help))
      (setq ac-quick-help
            (funcall (if (and ac-quick-help-prefer-tooltip ;; AJM remove X window specifics
                              window-system
                              (featurep 'pos-tip))
...
(defun ac-last-quick-help ()
  ...
  (when (and ac-last-completion
    ...
    (let ((doc (popup-item-documentation (cdr ac-last-completion)))
      ...
      (when (stringp doc)
        (if (and ac-quick-help-prefer-tooltip ;; AJM remove X window specifics
                 window-system
                 (featurep 'pos-tip))
