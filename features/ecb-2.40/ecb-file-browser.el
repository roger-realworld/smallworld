...
;;
;; Smallworld Bugfix:
;;   fixed ecb-check-directory-for-caching for handling ecb-cache-directory-contents-not
;; 
       ...
       (and (not (catch 'exit
		   (dolist (elem ecb-cache-directory-contents-not)
		     (let ((case-fold-search t))
		       (save-match-data
			 (if (string-match elem dir) ;;Smallworld fix
			     (throw 'exit elem)))    ;;Fix list of regexps for ecb-cache-directory-contents-not 
		       nil))))
	    (catch 'exit
