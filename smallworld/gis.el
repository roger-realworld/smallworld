...
(easy-menu-define gis-menu gis-mode-map
  ...
  `(,resources-gis-menu
    ...
    "----"
    [,resources-dev-tools-menu-oi        dev-tools-object-inspector    :active t :keys "f4 o, f2 X"]
    [,resources-dev-tools-menu-vsd-method dev-tools-vsd-transmit-method :active t :keys "f4 d"]
    [,resources-dev-tools-menu-tb        dev-tools-traceback-viewer    :active t :keys "f4 t"]
    "----"
...
(defconst gis-command-default
  (let ((on-nt (running-under-nt))
	(Mextdir-option ;use a 'TEMP' Environment variable if available
	 (cond ((getenv "TEMP") " -Mextdir $TEMP")
	       ((getenv "TMP")  " -Mextdir $TMP")
	       ((getenv "TMPDIR")  " -Mextdir $TMPDIR")
	       (t ""))))
    (format "[%%HOME%%] %s%s -image ${SMALLWORLD_GIS}/images/swaf.msf"
	    (if on-nt "sw_magik_win32" "sw_magik_motif")
	    Mextdir-option))
