...
(defconst magik-template-version "$Revision: 1.5 $")
...
Only the text in the template starting from a line matching ^# will be inserted."
  (if type   ;; to strip out the preamble from the template.
	...
	(goto-char (point-min))
	  ...
	  (error resources-template-no-header-error (magik-template-file type)))
  ...
  (let* ((type (or magik-template-file-type
	...
	(progn
	  (loop for type-data in magik-template-file-type-alist
		...
		for prompt        = (concat (format resources-template-type-prompt label) " ")
  ...
  (mapcar
   (lambda (al)
     (let* ((template-type (car al))
       ...
       (insert (format resources-template-copying-file source destination) "\n")
       (if (symbol-value 'mpde-debug) ;; simply to avoid compiler warnings
	   (insert "DEBUG: ")
	 (and (file-exists-p destination) (delete-file destination))
	 (copy-file source destination t))
       (insert (format resources-template-copied-file source destination) "\n")))
   alist))
