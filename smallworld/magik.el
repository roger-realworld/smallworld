;;; magik.el -- mode for editing Magik + some utils.
...
(eval-when-compile
  ...
  (defvar gis-buffer)
  (defvar gis-prompt)
  ;;Cannot require 'cb  because of circular dependency
  ...
  `(,resources-magik-menu
    ...
    [,resources-deep-print-menu-start        deep-print                     :active (and (fboundp 'deep-print)
											 (sw-buffer-mode-list 'gis-mode))
					                                   :keys "f2 x"]
    "---"
    [,resources-dev-tools-menu-oi            dev-tools-object-inspector     :active (sw-buffer-mode-list 'gis-mode) :keys "f4 o, f2 X"]
    [,resources-dev-tools-menu-vsd-method    dev-tools-vsd-transmit-method :active (and (or (not magik-method-name-mode)
											    (not (equal magik-method-name "")))
											(sw-buffer-mode-list 'gis-mode)) :keys "f4 d"]
    "---"
...
(defcustom magik-transmit-method-eom-mode nil
  ...
  :group  'magik
  :type  (list 'choice
	       (list 'const ':tag resources-magik-menu-option-eom-mode-off nil)
	       (list 'const ':tag resources-magik-menu-option-eom-mode-on 'end)
	       (list 'const ':tag resources-magik-menu-option-eom-mode-repeat 'repeat)))
  ...
  (save-excursion
    (let*
        ((toks (progn (insert ? )  ;; so that the token closes!
                      (prog1
                          (tokenise-magik-region-no-eol (point-bol) (point))
...
(defun magik-add-debug-statement ()
  ...
  (let
      ((var (sw-find-tag-default))
       ...
       tb)
    (save-excursion
      (or var (error resources-magik-no-current-variable-error))
      (backward-method)
      (push-mark nil t)
      (while
	  (not (or (eobp)
...
(defun magik-indent-command ()
  ...
  (let ((pragma-brackets (pragma-line-p)))
    (if (consp pragma-brackets)
	(electric-pragma-tab pragma-brackets)
      (magik-indent-line))))
...
you can use \\[isearch-forward-regexp] and use M-p to recall the search."
  (interactive
   (list
	...
	(read-string (concat resources-magik-goto-class-prompt ": ")
...
you can use \\[isearch-forward-regexp] and use M-p to recall this search."
  ...
  (let ((continue-p t)
	(pt (point))
	(prompt       (concat (format resources-magik-goto-class-method-loop-prompt arg) " "))
	(start-prompt (concat (format resources-magik-goto-class-method-loop-start-prompt arg) " "))
	next)
    (save-excursion
  ...
  (message 
   (cond ((null magik-transmit-method-eom-mode)
	 ...
	 ((eq magik-transmit-method-eom-mode 'end)
	  resources-magik-transmit-method-eom-on)
...
(defun magik-transmit-buffer ()
  ...
  (message resources-magik-code-loaded (or (buffer-file-name) (buffer-name))))
        ...
        (progn
          ...
          (while
              (and (not (eq (point) (point-max)))
                   (or (< (point) original-point)
                       stack))
            (dolist (tok (tokenise-magik-line))
              (cond
          ...
          (if (< (point) original-point)
              (progn
                ...
                (error resources-magik-transmit-error))
...
The location of point is determined by `magik-transmit-method-eom-mode'.
...
Otherwise, point is left where it is."
  ...
  (if (eq last-command 'magik-transmit-method-first) (forward-endmethod))
...
another file shall be written."
   (let* ((gis (sw-get-buffer-mode gis
	  ...
	  (filename (concat (if (running-under-nt)
...
You can customise magik-mode with the magik-mode-hook."
  ...
  (setq major-mode 'magik-mode
	mode-name resources-magik-menu
...
Toggles the value of `magik-method-name-mode'.
...
(defun magik-method-name-set ()
  "Sets the name of the current Magik method in `magik-method-name'."
...
(defun magik-heading ()
  ...
  (save-excursion
    (save-restriction
      (narrow-to-region (point-bol) (point-eol))
      (upcase-region (point-bol) (point-eol))
      (beginning-of-line)
      (forward-char 2)
      (delete-horizontal-space)
      (replace-regexp "\\(.\\)" " \\1")
      (end-of-line)
      (insert "\n" (buffer-substring-no-properties (point-bol) (point-eol)))
      (beginning-of-line)
      (replace-regexp "[^#]" "_")
      (forward-line -1)
      (replace-regexp "^\\(##?\\)." "\\1\t"))))

(defun fill-magik-public-comment ()
	...
	(let*
	    ((comment-str (match-string 1))
	     ...
	     (fill-prefix
	      (concat (progn
			(back-to-indentation)
			(buffer-substring-no-properties (point-bol) (point)))
		      comment-str
		      " "))
...
(defun sw:un-comment (nlines)
"Deletes # in first column of ARG lines at point"
  (interactive "*p")
    (while (and (> nlines 0) (not (eobp)))
       (decf nlines)
       (beginning-of-line 1)
...
"Puts # in first column of each line in the region"
  ...
  (save-excursion
    ...
    (sw:comment (count-lines (point) (mark t)))))

(defun magik-uncomment-region()
"Removes # in first column of each line in the region"
  (interactive "*")
  (save-excursion
    (if (> (point) (mark t)) (exchange-point-and-mark))
    (sw:un-comment (count-lines (point) (mark t)))))
...
With a prefix arg, ask user for GIS buffer to use."
  ...
  (if (equal (sw-curr-word) "")
      (message resources-magik-complete-empty-string)
    (if (<= (length (sw-curr-word)) 2)
        (message resources-magik-complete-string))
    (process-send-string
  ...
  (save-excursion
	...
	(forward-line 1))
      ...
      (cond ((eq (following-char) ?\()
		 ...
		 (error resources-magik-no-round-bracket-error)))
	    ((eq (following-char) ?\[)
	     (or (search-forward "]" nil t)
...
(defun magik-ecb-symboldef (symbol buffer)
  "Return Magik definitions for use in ECB symboldef buffer."
  (let ((global (car (member symbol magik-ac-global-source-cache)))
	(object (car (member symbol magik-ac-object-source-cache)))
	(condition (car (member (substring symbol 1) magik-ac-raise-condition-source-cache)))
	docstring
	retval)
    (if global
	(setq docstring (get-text-property 0 'document global)
	      retval "Magik Global"))
    (if object
	(setq docstring (concat symbol " is a Magik Object.")
	      retval "Magik Exemplar"))
    (if condition
	(setq docstring (get-text-property 0 'document condition)
	      retval "Magik Condition"))
    (if docstring
	(insert docstring "\n")
      (insert "*  No symbol definition can be found *\n"
	      "*  Have you started a GIS session?   *"))
    retval))
...
(or (assq 'transmit-magik-debug-p minor-mode-alist)
    (push '(transmit-magik-debug-p magik-transmit-debug-mode-line-string) minor-mode-alist))
...
(defun magik-msb-configuration ()
  "Adds Magik files to msb menu, supposes that msb is already loaded."
...
(eval-after-load 'ecb
  '(progn
     (add-to-list 'ecb-symboldef-find-functions '(magik-mode . magik-ecb-symboldef))
