...
(defconst menu-sw-dev-menu
  `(,resources-menu-sw-dev
    ...
    [,resources-menu-sw-dev-help       magik-patch-help
				       :active t] ;; :key-sequence nil
    "---"
    [,resources-mpde-menu-install      mpde-install
				       (and (boundp 'mpde-boot-installed)
					    (not mpde-boot-installed))]))
...
(defun menu-sw-set-menus ()
  ...
  (if (and (fboundp 'mpde-configurations-available-p)
	   (mpde-configurations-available-p))
      (easy-menu-change nil
			resources-menu-sw-dev
			(cdr menu-sw-dev-menu)))
