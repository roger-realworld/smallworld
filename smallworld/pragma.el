...
(defconst pragma-version "$Revision: 1.19 $")

(defvar topic-select-mode-map (make-keymap)
  "Keymap for Topic selection in _pragma lines")

(define-key topic-select-mode-map "y"  'topic-select-mark)
(define-key topic-select-mode-map "n"  'topic-select-unmark)
(define-key topic-select-mode-map "m"  'topic-select-mark)
(define-key topic-select-mode-map "u"  'topic-select-unmark)
(define-key topic-select-mode-map " "  'forward-line)
(define-key topic-select-mode-map "q"  'topic-select-quit)
(define-key topic-select-mode-map "\r" 'topic-select-select)
(define-key topic-select-mode-map "e"  'topic-edit)

;;;;;;;;;;;;;;;;;;;; User interface ;;;;;;;;;;;;;;;;;;;

(defun electric-pragma-tab (pragma-brackets)
...
which case we toggle back through the various pragma options."
  (interactive "*p")
  (pragma-electric-toggle-option arg 'forward))

(defun electric-pragma-back-/ (arg)
  "Insert the char, `\\', unless the current line starts with `_pragma', in
which case we toggle back through the various pragma options."
  (interactive "*p")
  (pragma-electric-toggle-option arg 'backward))
...
(defun pragma-do-if-match (list &optional default-elem reverse)
...
(defun pragma-if-match-replace-with-next (current next reverse)
...
(defvar pragma-magik-deprecated-action-list
  '(("Remove at next release."
     (looking-at " *Remove at next release. *")       pragma-if-match-replace-with-next)
    ("Restricted at next release."
     (looking-at " *Restricted at next release. *")   pragma-if-match-replace-with-next))
...
(defun pragma-magik-deprecated-action-toggle (direction)
  "toggle the current deprecated action option"
   (goto-char (match-end 0))
   (pragma-do-if-match pragma-magik-deprecated-action-list
		       '(default  (looking-at "<.*>") pragma-if-match-replace-with-next)
		       (eq direction 'backward)))

(defun pragma-goto-magik-deprecated-template ()
...
(defun pragma-insert-magik-deprecated-template ()
...
(save-excursion
  (save-match-data
    ...
    (and (pragma-goto-magik-deprecated-template)
	 (not (looking-at (concat "\\s-*" pragma-magik-deprecated-template-start)))
	 (let ((start (point))
	       (column (current-indentation))
	       (template (copy-sequence pragma-magik-deprecated-template)))
	   ...
	   (message resources-pragma-deprecated-template))))))

(defun pragma-remove-magik-deprecated-template ()
...
(save-excursion
  (search-forward ")") ; find end of _pragma statement
  (forward-line 1)
  (if (pragma-goto-magik-deprecated-template)
      (let ((start nil)
	    (end nil))
	(if (looking-at pragma-magik-deprecated-template-re)
	    ;;No changes made just remove whole template
	    (delete-region (match-beginning 0) (match-end 0))
	  (and (looking-at (concat "\\s-*" pragma-magik-deprecated-template-start))
	       (setq start (match-beginning 0)))
	  (setq end (re-search-forward (concat "\\s-*" pragma-magik-deprecated-template-end) nil t))
	  (and start
	       end
	       (y-or-n-p (concat resources-pragma-yn-deprecated-modify-comments " "))
	       (delete-region start end)))))))

;;;;;;;;;;;;;;;;;;;; Pragma toggle options ;;;;;;;;;;;;;;;;;;;

(defvar pragma-electric-toggle-list
      '(
	(classify-level (looking-at "c?lassify_level") pragma-if-match-insert-classify_level)
	(usage          (looking-at "u?sage")          pragma-if-match-insert-usage)
	(topic          (looking-at "t?opic")          pragma-if-match-do-the-electric-pragma-topics))
"The list used to control behaviour for the various fields in _pragma.
The format follows that described in pragma-do-if-match.")

(defvar pragma-classify_level-list
  '((basic      (looking-at " *basic *")      pragma-if-match-replace-with-next)
    (advanced   (looking-at " *advanced *")   pragma-if-match-replace-with-next)
    (restricted (looking-at " *restricted *") pragma-if-match-replace-with-next)
    (deprecated (looking-at " *deprecated *") pragma-if-match-replace-with-next)
    (debug      (looking-at " *debug *")      pragma-if-match-replace-with-next))
"The list used to control behaviour for the classify_level field in _pragma.
The format follows that described in pragma-do-if-match.")

(defvar pragma-usage-list
  '(("{subclassable}"
     (looking-at" *{ *subclassable *} *")                          pragma-if-match-replace-with-next)
    ("{redefinable}"
     (looking-at " *{ *redefinable *} *")                          pragma-if-match-replace-with-next)
    ("{redefinable, subclassable}"
     (looking-at " *{ *redefinable *, *subclassable *} *")         pragma-if-match-replace-with-next)
    ("{external}"
     (looking-at " *{ *external *} *")                             pragma-if-match-replace-with-next)
    ("{not_international}"
     (looking-at " *{ *not_international *} *")                    pragma-if-match-replace-with-next)
    ("{not_international, subclassable}"
     (looking-at " *{ *not_international *, *subclassable *} *")   pragma-if-match-replace-with-next)
    ("{not_international, redefinable}"
     (looking-at " *{ *not_international *, *redefinable *} *")    pragma-if-match-replace-with-next)
    ("{not_international, redefinable, subclassable}"
     (looking-at "{not_international, redefinable, subclassable}") pragma-if-match-replace-with-next))
"The list used to control behaviour for the usage field in _pragma.
The format follows that described in pragma-do-if-match.")

(defun pragma-electric-toggle-option (arg direction)
  "Insert the char, `/', unless the current line starts with `_pragma', in
which case we toggle through the various pragma options."
  (save-match-data
    (let ((pragma-brackets (pragma-line-p)))
      (cond ((consp pragma-brackets)
	     (let ((curr-pos (point))
		   (start-bracket (car pragma-brackets))
		   (end-bracket (cdr pragma-brackets))
		   option-pos)
		 ...
		 (search-forward "=" end-bracket t))              ; LOOP: skip passed current option
	       ...
	       (pragma-electric-toggle direction)))
	    ((save-excursion
	       (beginning-of-line)
	       (looking-at "\\s-*## Action\\s-+: "))
	     (pragma-magik-deprecated-action-toggle direction))
	    (t
	     (self-insert-command arg))))))

(defun pragma-electric-toggle (direction)
...
relative the current setting and available values."
  ...
  (pragma-do-if-match pragma-electric-toggle-list nil (eq direction 'backward)))
...
(defun pragma-if-match-insert-usage (current next reverse)
"Insert the usage according to the current setting."
;;Ensure point stays immediately after = by searching for = and doing the replace inside save-excursion
(search-forward "=")
(save-excursion
  (pragma-do-if-match pragma-usage-list
		      '(default (looking-at "{.*}") pragma-if-match-replace-with-next)
		      reverse)))
...
(defvar pragma-sw-window-configuration nil
"Window configuration to return to after topic selection mode.")

(defun pragma-if-match-do-the-electric-pragma-topics (current next reverse)
  ...
  (let* ((buffer-dir (if buffer-file-name (file-name-directory buffer-file-name) default-directory))
	 (pragma-files (if buffer-dir (sw-find-files-up buffer-dir "data/doc/pragma_topics")))
    ...
    (while
      ...
      (goto-char (match-end 0)))
    (goto-char pos)
    (setq pragma-sw-window-configuration
          (current-window-configuration))
    (pop-to-buffer "*topic-selection*")
    (erase-buffer)
    (topic-select-mode)
    (insert resources-pragma-topic-selection)
    (mapcar 'insert-file-contents pragma-files)
    (and product-pragma-file
	 (not (member product-pragma-file pragma-files))
	 (file-exists-p product-pragma-file)
	 (insert-file-contents product-pragma-file))
    (or pragma-files
	product-pragma-file
	(error resources-pragma-no-smallworld-gis-env))
    (goto-char
     (prog1
         (point)
...
(defun topic-select-mode ()
  ...
  (setq mode-name resources-pragma-topic-select)
...
(defun topic-select-quit ()
  ...
  (set-window-configuration pragma-sw-window-configuration))
...
(defun topic-edit ()
  ...
  (interactive)
  (let* ((buffer-dir (if buffer-file-name (file-name-directory buffer-file-name) default-directory))
	 (pragma-file (if buffer-dir (sw-find-files-up buffer-dir "data/doc/pragma_topics" t))))
    (cond (pragma-file
	   (find-file (car pragma-file)))
