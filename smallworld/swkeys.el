...
(defun sw-reload-dotemacs ()
  "Re-load all the Smallworld customisations by loading the 'load_sw_defs' file."
  (interactive)
  (load "load_sw_defs"))
...
(defun sw-set-keys ()
  ...
  (sw-multi-define-key (list sw-f2-map gis-f2-map magik-f2-map) "X"    'dev-tools-object-inspector)
  ...
  ;;TODO the following settings need to be set normally but the functions
  ;;need to become aware of the environment in which they can work.
  (if (featurep 'mpde-boot)
      (progn
	(if (mpde-master-available-p)
	    (progn 
	      (define-key sw-f2-map "i" 'magik-patch-submit)
	      (define-key sw-f2-map "j" 'magik-patch-bug-get)))
	(if (mpde-configurations-available-p)
	    (progn
	      (define-key sw-f2-map "d" 'magik-patch-make-change-note)
	      (define-key sw-f2-map "o" 'magik-patch-make)
	      (define-key sw-f2-map "u" 'magik-patch-make-unit-test)))))
  ...
  (define-key gis-f4-map "d"    'dev-tools-vsd-transmit-method)
  (define-key gis-f4-map "g"    'gis-error-goto)
  (define-key gis-f4-map "o"    'dev-tools-object-inspector)
  ...
  (define-key gis-f4-map "t"    'dev-tools-traceback-viewer)
  ...
  (define-key magik-f4-map "d"    'dev-tools-vsd-transmit-method)
  ...
  (define-key magik-f4-map "o"    'dev-tools-object-inspector)
