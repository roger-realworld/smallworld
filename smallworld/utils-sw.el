...
(defconst utils-sw-version "$Revision: 1.13 $")
...
(defvar gis-buffer) ; shut compiler up.
...
(defun point-bol ()
  "The value of the position at the beginning of the current line."
  (save-excursion
    (beginning-of-line)
    (point)))

(defun point-eol ()
  "The value of the position at the end of the current line."
  (save-excursion
    (end-of-line)
    (point)))
...
(defun sw-find-files-up (path file &optional first)
...
(defun sw-find-tag-tag (string)
  (let* ((default (sw-find-tag-default))
	 (spec (read-string
		(if default
...
(defun toggle-debug ()
  "Toggle the variables, `debug-on-error' and `debug-on-quit' on and off"
  (interactive)
  (setq debug-on-error (not debug-on-error))
  (setq debug-on-quit (not debug-on-quit))
  (message "debug-on-error %s, debug-on-quit %s"
           debug-on-error
           debug-on-quit))

(defun sw-substitute-in-string (string)
...
(defun sw-buffer-mode-list-predicate-p (predicate)
...
(defun sw-buffer-visible-list (mode &optional predicate)
...
Optional PREDICATE is either a function or a variable which must not return nil."
  (save-excursion
    (loop for b in (buffer-list)
	  ...
	  if (sw-buffer-mode-list-predicate-p predicate)
	  collect (cons (buffer-name)
			(windowp (get-buffer-window b nil))))))

(defun sw-buffer-mode-list (mode &optional predicate)
...
(defun sw-buffer-mode-list-sorted (mode &optional predicate sort-fn)
...
GIS buffers."
  (sort (sw-buffer-mode-list mode predicate)
	(or sort-fn 'string-lessp)))

(defun sw-get-buffer-mode (buffer mode prompt default &optional prefix-fn initial predicate)
...
"
  (let* ((prefix-fn (or prefix-fn
			'(lambda (arg mode predicate)
			   (nth (1- arg)
				(reverse (sw-buffer-mode-list-sorted mode predicate))))))
	 (prompt (concat prompt " "))
	 (visible-bufs (sw-buffer-visible-list mode predicate))
	 bufs
...
(defun sw-global-set-key (keys function)
  "Like \\[global-set-key] but translates GNU Emacs C-foo syntax into XEmacs (control foo) syntax."
